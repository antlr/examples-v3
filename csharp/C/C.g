...
This grammar requires ANTLR v3.2.1 or higher.
...
scope Symbols {
	IDictionary<object, object> types;
}
...
{
	// Line below requires LINQ. Stack enumerator works from top to bottom.
	//return Symbols_stack.Any(i => ((Symbols_scope)i).types.ContainsKey(name));
	foreach (Symbols_scope s in Symbols_stack) {
		if (s.types.ContainsKey(name)) {
			return true;
		}
	}
	return false;
	
	// Java version would require indexer. If missing indexer is a problem please report to list!
