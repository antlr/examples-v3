...
CLASS
	:	'class' WS name=ID WS? ('extends' WS QID WS?)?
        ...
        {Console.Out.WriteLine("found class "+$name.Text);}
	;
	
METHOD
    :   TYPE WS name=ID WS? '(' ( ARG WS? (',' WS? ARG WS?)* )? ')' WS? 
       ('throws' WS QID WS? (',' WS? QID WS?)*)? '{'
        {Console.Out.WriteLine("found method "+$name.Text);}
    ;

FIELD
    :   TYPE WS name=ID '[]'? WS? (';'|'=')
        {Console.Out.WriteLine("found var "+$name.Text);}
    ;
...
CALL
    :   name=QID WS? '('
        {/*ignore if this/super */ Console.Out.WriteLine("found call "+$name.Text);}
    ;
