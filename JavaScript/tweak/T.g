...
@members {
	this.tokens; // avoid typecasts all over
}
...
@init {
    ...
    :   method+
        {
        this.tokens.insertBefore(start,"public class Wrapper {\n");
        // note the reference to the last token matched for method:
        this.tokens.insertAfter($method.stop, "\n}\n");
        }
    ;

method
    :   m='method' ID '(' ')' body
        {this.tokens.replace($m, "public void");}
    ; 
...
@init {
    ...
    :   lcurly='{' stat* '}'
        ...
        for (var i =0; i < $body::decls.length; i++) {
            this.tokens.insertAfter($lcurly, "\nint "+$body::decls[i]+";");
        }
        }
    ;

stat:   ID '=' expr ';' {$body::decls.push($ID.text);} // track left-hand-sides
    ;
