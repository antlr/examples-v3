...
scope Symbols {
	Object types; // only track types in order to get parser working
}

@members {
	private function isTypeName(name:String):Boolean {
		for (var i:int = Symbols_stack.length-1; i>=0; i--) {
			var scope:Object = Symbols_stack[i];
			if ( scope.types.hasOwnProperty(name) ) {
				return true;
			}
		}
		return false;
...
direct_declarator
	:   (	IDENTIFIER
			...
			if ($declaration.length>0&&$declaration::isTypedef) {
				var types:Object = $Symbols::types;
				var symbol:String = $IDENTIFIER.text;
				if (types != null && symbol != null) {
					types[symbol] = true;
					trace("define type "+ symbol);
				}
			}
			}
		|	'(' declarator ')'
