...
@lexer::members {
public static const JAVADOC_CHANNEL:int = 1;
...
variable: 'int' ID ('=' expr)? ';'
        ;
method  : 'method' ID '(' ')' {trace("enter method "+$ID.text);}
          block
        ;
...
statement
        : ID '=' expr ';' {trace("assignment to "+$ID.text);}
        | 'return' expr ';'
        | block
        ;
...
RCURLY  : '}'
          {
          if ( nesting<=0 ) {
                emitToken(TokenConstants.EOF_TOKEN);
                trace("exiting embedded simple");
          }
          else {
                nesting--;
          }
...
JAVADOC : '/**'
          {
            ...
            trace("enter javadoc");
            var j:JavadocLexer = new JavadocLexer(input);
            var tokens:CommonTokenStream = new CommonTokenStream(j);
            tokens.discardTokenType(JavadocLexer.WS);
            var p:JavadocParser = new JavadocParser(tokens);
            p.comment();
