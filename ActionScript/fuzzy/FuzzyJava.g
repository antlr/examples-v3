...
options {language=ActionScript;filter=true;}
...
CLASS
	:	'class' WS name=ID WS? ('extends' WS QID WS?)?
        ...
        {trace("found class "+$name.text);}
	;
	
METHOD
    :   TYPE WS name=ID WS? '(' ( ARG WS? (',' WS? ARG WS?)* )? ')' WS? 
       ('throws' WS QID WS? (',' WS? QID WS?)*)? '{'
        {trace("found method "+$name.text);}
    ;

FIELD
    :   TYPE WS name=ID '[]'? WS? (';'|'=')
        {trace("found var "+$name.text);}
    ;
...
CALL
    :   name=QID WS? '('
        {/*ignore if this/super */ trace("found call "+$name.text);}
    ;
