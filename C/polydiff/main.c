// Example of a grammar for multiple tree parsers that rewrite the tree.
// Adapted from Java equivalent example, by Terence Parr
// Author: Jim Idle - Feb 2008
...
//        provided in ../vsrulefiles/antlrXXX.rules
//        there is no reason that this MUST be the case. There is also a GNU Makefile
//		  in each example directory and a master Makefile in the main C examples top
//		  level directory.
//
...
//		  Hence you can generate on say Windows, and just use the generated C files to compile
//		  directly on systems where Java is not very good, such as HPUX.
//
...
main	(int argc, char *argv[])
{
    ...
    //
    PolyParser_poly_return				polyAST;
	PolyDifferentiator_poly_return		polyDiffAST;
	Simplifier_poly_return				simplAST;
    ...
    // the tree parser
    //
    pPolyDifferentiator		    treeDiff;
	pSimplifier					treeSimp;
	pPolyPrinter				treePrint;
    ...
    //
    lxr	    = PolyLexerNew(input);	    // CLexerNew is generated by ANTLR
    ...
    //
    psr	    = PolyParserNew(tstream);  // CParserNew is generated by ANTLR3
	...
	{
		printf("Polynomial tree: \n%s\n\n", polyAST.tree->toStringTree(polyAST.tree)->chars);

		// Create the tree node stream
		//
		nodes	= antlr3CommonTreeNodeStreamNewTree(polyAST.tree, ANTLR3_SIZE_HINT); // sIZE HINT WILL SOON BE DEPRECATED!!

		// Differentiate
		//
		treeDiff = PolyDifferentiatorNew(nodes);
		polyDiffAST = treeDiff->poly(treeDiff);

		if (treeDiff->pTreeParser->rec->state->errorCount > 0)
		{
			fprintf(stderr, "The differentiator returned %d errors, tree walking aborted.\n", treeDiff->pTreeParser->rec->state->errorCount);
			exit(5);
		}
		else
		{
