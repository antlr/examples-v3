...
// July 2006
// 
// ANTLR Delphi version - Erik van Bilsen, October 2008
grammar C;
...
var
  ...
  Scope: TSymbolsScope;
begin
  for I := SymbolsStack.Count - 1 downto 0 do
  begin
    Scope := TSymbolsScope(SymbolsStack[I].Implementor);
    if (Scope.types.ContainsKey(name)) then
      Exit(True);
  end;
  Result := False;
end;
...
@init {
  $Symbols::types = TDictionary<string, string>.Create;
}
...
direct_declarator
	:   (	IDENTIFIER
			{
			if ($declaration.Count>0) and ($declaration::isTypedef) then
			begin
			  $Symbols::types.AddOrSetValue($IDENTIFIER.Text, $IDENTIFIER.Text);
