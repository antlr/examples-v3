...
/* Cannot appear *before* "options {...}", "tokens {...}" et al... */
...
P: IJavadocParser;
} : '/**'
          {
            ...
            J := TJavadocLexer.Create(Input);
            Tokens := TCommonTokenStream.Create(J);
            Tokens.DiscardTokenType(TJavadocLexer.WS);
            P := TJavadocParser.Create(Tokens);
            P.comment();
